...
object RecursiveData {
  ...
  def testListIntEmpty(list: List[Int]): Boolean = ListIntEmpty(list: List[Int])
  ...
  // используйте функцию из пункта (a) здесь, не изменяйте сигнатуру
  def testListIntHead(list: List[Int]): Int = ListIntHead(list: List[Int])
  ...
  def main(args: Array[String]):Unit = {
    ...
    println(RecursiveData.testListIntHead(Cons(1, Cons(2, Cons(3, Nil())))))
